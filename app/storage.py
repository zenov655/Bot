...
# На Bothost постоянное хранилище данных между обновлениями из Git — папка /app/data.
# Если такой папки нет (локальный запуск/тест), база лежит в корне проекта, как раньше.
_HOST_DATA_DIR = Path("/app/data")
if _HOST_DATA_DIR.is_dir():
    DB_PATH = _HOST_DATA_DIR / "orders.db"
else:
